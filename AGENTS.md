...
An Ansible collection (`community.proxysql`) providing modules for managing [ProxySQL](https://www.proxysql.com/) — a high-performance MySQL/PostgreSQL proxy. No roles exist — only modules and shared utilities.

## Development Environment

The collection must reside at `ansible_collections/community/proxysql/` (relative to a directory on `ANSIBLE_COLLECTIONS_PATHS`) for imports to resolve correctly.
...
- Do not add connection parameters to individual modules. Extend the `proxysql` doc fragment in `plugins/doc_fragments/proxysql.py` instead.
...
- Use conventional commit message prefixes: `feat:`, `fix:`, `docs:`, `test:`, `refactor:`, `chore:`, `ci:`. Example: `fix: handle empty hostgroup list in proxysql_info`.
