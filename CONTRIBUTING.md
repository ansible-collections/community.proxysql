...
Whether you are looking for an opportunity to contribute or you found a bug and already know how to solve it, please go to the [issue tracker](https://github.com/ansible-collections/community.proxysql/issues).
...
Look through currently [open pull requests](https://github.com/ansible-collections/community.proxysql/pulls).
...
5. Fork the `community.proxysql` repository through the GitHub web interface.
...
```bash
git clone git@github.com:YOURACC/community.proxysql.git ~/ansible_collections/community/proxysql
```

7. Go to your new cloned repository.
```bash
cd ~/ansible_collections/community/proxysql
```
...
For example, if you are fixing the `proxysql_mysql_users` module, its tests are in `tests/integration/targets/test_proxysql_mysql_users/tasks`.
...
15. Run `flake8` against a changed file. If it is `plugins/modules/proxysql_mysql_users.py`:
```bash
flake8 plugins/modules/proxysql_mysql_users.py
```
...
git commit -m "proxysql_mysql_users: fix crash when ..."
```
...
20. Go to the `upstream` (http://github.com/ansible-collections/community.proxysql).
