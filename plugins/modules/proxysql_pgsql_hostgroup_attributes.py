...
DOCUMENTATION = '''
---
module: proxysql_pgsql_hostgroup_attributes
author: "Janos Ruszo (@jruszo)"
short_description: Manages PostgreSQL hostgroup attributes using the ProxySQL admin
                   interface
description:
   - Each row in pgsql_hostgroup_attributes provides a per-hostgroup override
...
RETURN = '''
stdout:
    description: The pgsql_hostgroup_override modified or removed from
                 ProxySQL.
    ...
    "sample": {
        "changed": true,
        "msg": "Added entry to pgsql_hostgroup_overrides",
...
class ProxySQLPgSQLHostgroupAttributes(object):
    ...
    def create(self, result, cursor):
        if not self.check_mode:
            ...
            result['msg'] = "Added entry to pgsql_hostgroup_attributes"
            result['pgsql_hostgroup_attributes'] = self.select(cursor)
            self.manage_config(cursor, result['changed'])
        else:
            result['changed'] = True
            result['msg'] = ("Hostgroup attributes would have been added to" +
                             " pgsql_hostgroup_attributes, however" +
    ...
    def delete(self, result, cursor):
        if not self.check_mode:
            ...
            result['msg'] = "Deleted entry from pgsql_hostgroup_attributes"
            self.manage_config(cursor, result['changed'])
        else:
            result['changed'] = True
    ...
    def update(self, result, cursor):
        if not self.check_mode:
            ...
            result['msg'] = "Updated entry in pgsql_hostgroup_attributes"
            result['pgsql_hostgroup_attributes'] = self.select(cursor)
            self.manage_config(cursor, result['changed'])
        else:
            result['changed'] = True
    ...
    def _update(self, cursor):
        ...
        query = \
            "UPDATE pgsql_hostgroup_attributes SET " \
    ...
    if hostgroup_attributes.state == "present":
        ...
        try:
            if hostgroup_attributes.check_exists(cursor, keys=True):
                ...
                result['changed'] = False
                result['msg'] = ("The pgsql_hostgroup_attributes is absent" +
                                 " from pgsql_hostgroup_attributes memory" +
                                 " configuration")
