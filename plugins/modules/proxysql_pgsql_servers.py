...
DOCUMENTATION = '''
---
module: proxysql_pgsql_servers
author: "Janos Ruszo (@jruszo)"
short_description: Adds or removes pgsql hosts from proxysql admin interface
description:
...
options:
  hostgroup_id:
    description:
      - The hostgroup in which this PostgreSQL instance is included. An instance
  ...
  port:
    description:
      - The port at which the PostgreSQL instance can be contacted.
    type: int
    default: 5432
  status:
    description:
      - ONLINE - Backend server is fully operational.
        SHUNNED - The server is temporarily taken out of use because either too
                  many connection errors happened in a short time, or
    ...
    type: str
    choices: [ "ONLINE", "SHUNNED", "OFFLINE_SOFT", "OFFLINE_HARD"]
  weight:
    description:
  ...
  compression:
    description:
      - If I(compression) is set to C(1), new connections to that server will
        use compression. If omitted the proxysql database default for
        I(compression) is 0.
    type: int
  max_connections:
    description:
...
RETURN = '''
stdout:
    description: The pgsql host modified or removed from proxysql
    ...
    "sample": {
        ...
        "msg": "Added server to pgsql_servers",
        "server": {
            "comment": "",
            "compression": "0",
            "hostgroup_id": "1",
...
def perform_checks(module):
    ...
    if module.params["compression"]:
        if module.params["compression"] < 0 \
           or module.params["compression"] > 1:
            module.fail_json(
                msg="compression must be set between 0 and 1"
            )
    ...
    if module.params["max_replication_lag"]:
        if module.params["max_replication_lag"] < 0 \
           or module.params["max_replication_lag"] > 126144000:
            module.fail_json(
                msg="max_replication_lag must be set between 0 and 126144000"
            )


class ProxySQLPgSQLServer(object):
    ...
    def create_server(self, check_mode, result, cursor):
        if not check_mode:
            ...
            result['msg'] = "Added server to pgsql_servers"
            result['server'] = \
            ...
            result['msg'] = ("Server would have been added to" +
                             " pgsql_servers, however check_mode" +
                             " is enabled.")
    ...
    def update_server(self, check_mode, result, cursor):
        if not check_mode:
            ...
            result['msg'] = "Updated server in pgsql_servers"
            result['server'] = \
                self.get_server_config(cursor)
            self.manage_config(cursor,
                               result['changed'])
    ...
    def delete_server(self, check_mode, result, cursor):
        if not check_mode:
            ...
            result['msg'] = "Deleted server from pgsql_servers"
            self.manage_config(cursor,
                               result['changed'])
        else:
            result['changed'] = True
            result['msg'] = ("Server would have been deleted from" +
                             " pgsql_servers, however check_mode is" +
                             " enabled.")
...
def main():
    ...
    proxysql_server = ProxySQLPgSQLServer(module)
    result = {}
    ...
    if proxysql_server.state == "present":
        try:
            if not proxysql_server.check_server_config(cursor):
                ...
                result['msg'] = ("The server already exists in pgsql_servers" +
        ...
        try:
            if proxysql_server.check_server_config_exists(cursor):
                ...
                result['msg'] = ("The server is already absent from the" +
                                 " pgsql_servers memory configuration")
        except mysql_driver.Error as e:
            module.fail_json(
