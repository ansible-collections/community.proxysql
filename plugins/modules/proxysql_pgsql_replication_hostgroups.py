...
DOCUMENTATION = '''
---
module: proxysql_pgsql_replication_hostgroups
author: "Janos Ruszo (@jruszo)"
short_description: Manages replication hostgroups using the proxysql admin
                   interface
description:
   - Each row in pgsql_replication_hostgroups represent a pair of
...
options:
  ...
  check_type:
    description:
      - type of check to perform. Right now only C(read_only) is supported, so the field is unused.
    type: str
    choices: [ "read_only" ]
    default: read_only
  comment:
...
EXAMPLES = '''
---
# This example adds a replication hostgroup, it saves the pgsql server config
# to disk, but avoids loading the pgsql server config to runtime (this might be
...
# This example removes a replication hostgroup, saves the pgsql server config
# to disk, and dynamically loads the pgsql server config to runtime.  It uses
...
class ProxySQLPgSQLReplicationHostgroup(object):
    ...
    def check_repl_group_config(self, cursor, keys):
        query_string = \
            """SELECT count(*) AS `repl_groups`
               FROM pgsql_replication_hostgroups
    ...
    def manage_config(self, cursor, state):
        if state and not self.check_mode:
            if self.save_to_disk:
                save_config_to_disk(cursor, "SERVERS", "pgsql")
            if self.load_to_runtime:
                load_config_to_runtime(cursor, "SERVERS", "pgsql")

    def create_repl_group(self, result, cursor):
        if not self.check_mode:
            result['changed'] = \
                self.create_repl_group_config(cursor)
            result['msg'] = "Added repl group configuration to pgsql_replication_hostgroups"
            result['repl_group'] = \
            ...
            result['msg'] = ("Repl group would have been added to" +
                             " pgsql_replication_hostgroups, however" +
    ...
    def delete_repl_group(self, result, cursor):
        if not self.check_mode:
            ...
            result['msg'] = "Deleted repl group configuration from pgsql_replication_hostgroups"
            self.manage_config(cursor,
                               result['changed'])
        else:
            result['changed'] = True
    ...
    def update_check_type(self, cursor):
        try:
            query_string = ("UPDATE pgsql_replication_hostgroups "
                            "SET check_type = %s "
                            "WHERE writer_hostgroup = %s")
        ...
        try:
            if proxysql_repl_group.check_repl_group_config(cursor,
                ...
                result['msg'] = ("The repl group is already absent from the" +
                                 " pgsql_replication_hostgroups memory" +
                                 " configuration")
