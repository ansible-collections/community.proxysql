...
DOCUMENTATION = '''
---
module: proxysql_pgsql_query_rules
author: "Janos Ruszo (@jruszo)"
short_description: Modifies pgsql query rules using the proxysql admin interface
description:
...
options:
  ...
  database:
    description:
      - Filtering criteria matching database. If I(database) is non-NULL, a
        query will match only if the connection uses database as its default
        schema.
    type: str
  flagIN:
    description:
  ...
  digest:
    description:
      - Match queries with a specific digest, as returned by
        stats_pgsql_query_digest.digest.
    type: str
  match_digest:
    description:
  ...
  timeout:
    description:
      - The maximum timeout in milliseconds with which the matched or rewritten
        ...
        threshold, the query is automatically killed.
    type: int
  retries:
    description:
      - The maximum number of times a query needs to be re-executed in case of
        detected failure during the execution of the query.
    type: int
  delay:
    description:
      - Number of milliseconds to delay the execution of the query. This is
        ...
        priority to queries over others.
    type: int
  next_query_flagIN:
    description:
...
RETURN = '''
stdout:
    description: The pgsql query rule modified or removed from proxysql.
    ...
    "sample": {
        "changed": true,
        "msg": "Added rule to pgsql_query_rules",
        "rules": [
            {
                "active": "0",
                "apply": "0",
...
class ProxySQLPgSQLQueryRule(object):
    ...
    def create_rule(self, check_mode, result, cursor):
        if not check_mode:
            ...
            result['msg'] = "Added rule to pgsql_query_rules"
    ...
    def update_rule(self, check_mode, result, cursor):
        if not check_mode:
            ...
            result['msg'] = "Updated rule in pgsql_query_rules"
            self.manage_config(cursor,
                               result['changed'])
            result['rules'] = \
    ...
    def delete_rule(self, check_mode, result, cursor):
        if not check_mode:
            ...
            result['msg'] = "Deleted rule from pgsql_query_rules"
            self.manage_config(cursor,
                               result['changed'])
        else:
            result['changed'] = True
        ...
        try:
            if not proxysql_query_rule.check_rule_cfg_exists(cursor):
                ...
                result['msg'] = ("The rule already exists in" +
                                 " pgsql_query_rules and doesn't need to be" +
                                 " updated.")
                result['rules'] = \
        ...
        try:
            ...
            if existing_rules > 0:
                ...
                result['msg'] = ("The rule is already absent from the" +
                                 " pgsql_query_rules memory configuration")
        except mysql_driver.Error as e:
            module.fail_json(
