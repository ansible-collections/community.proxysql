---
name: pr-review
description: Reviews pull requests and code changes in the community.proxysql Ansible collection against project standards and the Ansible Collection Review Checklist. Use when asked to review a PR, patch, diff, or set of code changes. Do not use for GitHub Issues or general Q&A.
---
...
Review pull requests and code changes in the `community.proxysql` Ansible collection.
...
- New Python dependencies are documented appropriately.
...
- Module file names follow the `proxysql_<noun>` pattern.
...
- Connection and query utilities from `module_utils/mysql.py` are used for ProxySQL admin interface interactions.
